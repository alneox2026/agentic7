...
variable "billing_reconciler_service_account_name" {
  ...
  type        = string
  default     = "agentic7-reconciler-sa"
}
...
# The Cloud Shell deployment script supplies distinct values for every
# Firestore collection using FIRESTORE_NAMESPACE (derived from the repository
# directory). Keep that behavior when copying this template; manual Terraform
# users must set a unique value for every collection variable below.
variable "firestore_threads_collection" {
  description = "Top-level Firestore collection for chat threads. Cloud Shell overrides this with a stack-scoped name."
  type        = string
  default     = "agentic7_threads"
}

variable "firestore_messages_subcollection" {
  description = "Subcollection name for messages under each thread document."
  type        = string
  default     = "messages_agentic7"
}
...
variable "firestore_idempotency_collection" {
  ...
  type        = string
  default     = "processed_events_agentic7"
}
...
variable "firestore_billing_ledger_collection" {
  ...
  type        = string
  default     = "agent_billing_ledger_agentic7"
}
...
variable "firestore_customer_wallets_collection" {
  ...
  type        = string
  default     = "customer_wallets_agentic7"
}
...
variable "firestore_billing_reservations_collection" {
  ...
  type        = string
  default     = "billing_reservations_agentic7"
}
...
variable "firestore_wallet_transactions_collection" {
  ...
  type        = string
  default     = "wallet_transactions_agentic7"
}
...
variable "firestore_customer_billing_periods_collection" {
  ...
  type        = string
  default     = "customer_billing_periods_agentic7"
}
...
variable "firestore_customer_billing_accounts_collection" {
  ...
  type        = string
  default     = "customer_billing_accounts_agentic7"
}
...
variable "firestore_stripe_webhook_events_collection" {
  ...
  type        = string
  default     = "stripe_webhook_events_agentic7"
}
...
variable "billing_api_stripe_webhook_signing_secret_id" {
  ...
  type        = string
  default     = "stripe-webhook-signing-secret-agentic7"
}
...
variable "firestore_subscription_cancellation_requests_collection" {
  description = "Top-level private Firestore collection for subscription cancellation intents. Cloud Shell overrides this with a stack-scoped name."
  type        = string
  default     = "subscription_cancellation_requests_agentic7"
}
